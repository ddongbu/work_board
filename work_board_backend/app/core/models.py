...
class User(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, Identity(always=True, start=1, increment=1), primary_key=True)
...
class UserPassword(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, Identity(always=True, start=1, increment=1), primary_key=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey('app.user.id'), nullable=False)
...
class PostLike(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, Identity(always=True, start=1, increment=1), primary_key=True)
    post_id: Mapped[int] = mapped_column(Integer, ForeignKey('app.post.id', ondelete='CASCADE'), nullable=False)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey('app.user.id', ondelete='CASCADE'), nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=text('NOW()'))
...
class Comment(Base):
    ...
    post_id: Mapped[int] = mapped_column(Integer, ForeignKey('app.post.id', ondelete='CASCADE'), nullable=False)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey('app.user.id', ondelete='CASCADE'), nullable=False)
    parent_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('app.comment.id', ondelete='CASCADE'), nullable=True)
    content: Mapped[str] = mapped_column(Text, nullable=False)
    created_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=text('NOW()'))
    updated_at: Mapped[datetime.datetime] = mapped_column(DateTime, server_default=text('NOW()'))
